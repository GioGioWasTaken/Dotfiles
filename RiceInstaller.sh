...
install_dependencies() {
	...
	if [ -n "$(printf "%s" "$missing_pkgs" | tr -s ' ')" ]; then
		...
		printf "Installing %s packages total.\n" "$count"

		if sudo pacman -S --noconfirm $missing_pkgs 2>&1 | tee -a "$ERROR_LOG" ; then
...
clone_dotfiles() {
    ...
    if [ -d "$repo_dir" ]; then
        printf "GioDotfiles directory already exists.\n"
        read -r -p "Use existing files? [Y/n] " response
        response=${response,,}  # convert to lowercase

        if [[ "$response" == "y" || "$response" == "" ]]; then
            printf "Using existing dotfiles directory, skipping clone.\n"
            return 0
        else
            printf "Nuking existing GioDotfiles directory...\n"
            rm -rf "$repo_dir"
        fi
    fi
    ...
    if git clone "$repo_url" "$repo_dir" >> "$ERROR_LOG" 2>&1; then
        printf "Successfully cloned dotfiles.\n"
    else
...
copy_dotfiles() {
	...
	cd "$config_dir"
	printf "Copying all configs from $PWD to $HOME/.config"
	...
	for config in $config_list; do
		printf "Copying %s to ~/.config... (if it exists it will be replaced)\n" $config 
